...
!C     DUE TOC DORMAND & PRINCE (WITH STEPSIZE CONTROL AND
!C     DENSE OUTPUT)
!C
...
!C     INPUT PARAMETERS  
!C     ----------------  
!C     N           CDIMENSION OF THE SYSTEM 
!C
!C     FCN         NAME (EXTERNAL) OF CSUBROUTINE COMPUTING THE
!C                 VALUE OF F(X,Y):
...
!C                 CAN BE BOTH SCALARS OR CELSE BOTH VECTORS OF LENGTH N.
...
!C -------- NMAX , THE MAXIMAL NUMBER OF STEPS ----- 
      ...
      ELSE
         NMAX=IWORK(1)
         IF(NMAX.LE.0)THEN
            IF (IPRINT.GT.0) WRITE(IPRINT,*),' WRONG INPUT IWORK(1)=',IWORK(1)
 !              WRITE(IPRINT,*),' WRONG INPUT IWORK(1)=',IWORK(1)
            ARRET=.TRUE.
         END IF
      END IF
!C -------- METH   COEFFICIENTS OF THE METHOD
      ...
      ELSE
         ...
         IF(METH.LE.0.OR.METH.GE.4)THEN
            IF (IPRINT.GT.0) WRITE(IPRINT,*),' CURIOUS INPUT IWORK(2)=',IWORK(2)
...
!C -------- NRDENS   NUMBER OF DENSE OUTPUT COMPONENTS
      ...
      IF(NRDENS.LT.0.OR.NRDENS.GT.N)THEN
         IF (IPRINT.GT.0) WRITE(IPRINT,*),' CURIOUS INPUT IWORK(5)=',IWORK(5)
...
!C -------- UROUND   SMALLEST NUMBER SATISFYING 1.D0+UROUND>1.D0  
      ...
      ELSE
         ...
         IF(UROUND.LE.1.D-35.OR.UROUND.GE.1.D0)THEN
            IF (IPRINT.GT.0) WRITE(IPRINT,*),' WHICH MACHINE DO YOU HAVE? YOUR UROUND WAS:',WORK(1)
...
!C -------  SAFETY FACTOR -------------
      ...
      ELSE
         ...
         IF(SAFE.GE.1.D0.OR.SAFE.LE.1.D-4)THEN
            IF (IPRINT.GT.0) WRITE(IPRINT,*),' CURIOUS INPUT FOR SAFETY FACTOR WORK(2)=',WORK(2)
      ...
      ELSE
         ...
         ELSE
            ...
            IF(BETA.GT.0.2D0)THEN
               IF (IPRINT.GT.0) WRITE(IPRINT,*),' CURIOUS INPUT FOR BETA: WORK(5)=',WORK(5)
...
!C ------ TOTAL STORAGE REQUIREMENT -----------
      ...
      IF(ISTORE.GT.LWORK)THEN
        IF (IPRINT.GT.0) WRITE(IPRINT,*),' INSUFFICIENT STORAGE FOR WORK, MIN. LWORK=',ISTORE
!     &   ' INSUFFICIENT STORAGE FOR WORK, MIN. LWORK=',ISTORE
      ...
      IF(ISTORE.GT.LIWORK)THEN
        IF (IPRINT.GT.0) WRITE(IPRINT,*),' INSUFFICIENT STORAGE FOR IWORK, MIN. LIWORK=',ISTORE
...
!C --------! CALL TO CORE INTEGRATOR ------------
      CALL DP86CO(N,FCN,X,Y,XEND,HMAX,H,RTOL,ATOL,ITOL,IPRINT,SOLOUT,IOUT,IDID,NMAX,UROUND,METH,NSTIFF,SAFE,BETA,FAC1,FAC2,WORK(IEK1),WORK(IEK2),WORK(IEK3),WORK(IEK4),WORK(IEK5),WORK(IEK6),WORK(IEK7),WORK(IEK8),WORK(IEK9),WORK(IEK10),WORK(IEY1),WORK(IECO),IWORK(ICOMP),NRDENS,RPAR,IPAR,NFCN,NSTEP,NACCPT,NREJCT)
...
!C
      SUBROUTINE DP86CO(N,FCN,X,Y,XEND,HMAX,H,RTOL,ATOL,ITOL,IPRINT,SOLOUT,IOUT,IDID,NMAX,UROUND,METH,NSTIFF,SAFE,BETA,FAC1,FAC2,K1,K2,K3,K4,K5,K6,K7,K8,K9,K10,Y1,CONT,ICOMP,NRD,RPAR,IPAR,NFCN,NSTEP,NACCPT,NREJCT)
!     &   SOLOUT,IOUT,IDID,NMAX,UROUND,METH,NSTIFF,SAFE,BETA,FAC1,FAC2,
...
!     &  c16 = 0.777777777777777777777777777778D+00)
      parameter (b1 =   5.42937341165687622380535766363D-2,b6 =   4.45031289275240888144113950566D0,b7 =   1.89151789931450038304281599044D0,b8 =  -5.8012039600105847814672114227D0,b9 =   3.1116436695781989440891606237D-1,b10 = -1.52160949662516078556178806805D-1,b11 =  2.01365400804030348374776537501D-1,b12 =  4.47106157277725905176885569043D-2)
...
!     &  b12 =  4.47106157277725905176885569043D-2)
      parameter (bhh1 = 0.244094488188976377952755905512D+00,bhh2 = 0.733846688281611857341361741547D+00,bhh3 = 0.220588235294117647058823529412D-01)
!     &  bhh1 = 0.244094488188976377952755905512D+00,bhh2 = 0.733846688281611857341361741547D+00,bhh3 = 0.220588235294117647058823529412D-01)
!     &  bhh2 = 0.733846688281611857341361741547D+00,
!     &  bhh3 = 0.220588235294117647058823529412D-01)
      parameter (er1 =  0.1312004499419488073250102996D-01,er6 = -0.1225156446376204440720569753D+01,er7 = -0.4957589496572501915214079952D+00,er8 =  0.1664377182454986536961530415D+01,er9 = -0.3503288487499736816886487290D+00,er10 =  0.3341791187130174790297318841D+00,er11 =  0.8192320648511571246570742613D-01,er12 = -0.2235530786388629525884427845D-01)
...
!     &  er12 = -0.2235530786388629525884427845D-01)
      parameter (a21 =    5.26001519587677318785587544488D-2,a31 =    1.97250569845378994544595329183D-2,a32 =    5.91751709536136983633785987549D-2,a41 =    2.95875854768068491816892993775D-2,a43 =    8.87627564304205475450678981324D-2,a51 =    2.41365134159266685502369798665D-1,a53 =   -8.84549479328286085344864962717D-1,a54 =    9.24834003261792003115737966543D-1,a61 =    3.7037037037037037037037037037D-2,a64 =    1.70828608729473871279604482173D-1,a65 =    1.25467687566822425016691814123D-1,a71 =    3.7109375D-2,a74 =    1.70252211019544039314978060272D-1,a75 =    6.02165389804559606850219397283D-2,a76 =   -1.7578125D-2)
...
!     &  a1211 =  6.43392746015763530355970484046D-1)
      parameter (a141 =  5.61675022830479523392909219681D-2,a147 =  2.53500210216624811088794765333D-1,a148 = -2.46239037470802489917441475441D-1,a149 = -1.24191423263816360469010140626D-1,a1410 =  1.5329179827876569731206322685D-1,a1411 =  8.20105229563468988491666602057D-3,a1412 =  7.56789766054569976138603589584D-3,a1413 = -8.298D-3)
!     &  a141 =  5.61675022830479523392909219681D-2,a147 =  2.53500210216624811088794765333D-1,a148 = -2.46239037470802489917441475441D-1,a149 = -1.24191423263816360469010140626D-1,a1410 =  1.5329179827876569731206322685D-1,a1411 =  8.20105229563468988491666602057D-3,a1412 =  7.56789766054569976138603589584D-3,a1413 = -8.298D-3)
...
!     &  a1615 = -9.15095847217987001081870187138D0)
 ...
 !    &  d616 =  0.11992291136182789328035130030D+02)
      parameter (d71  = -0.25693933462703749003312586129D+02,d76  = -0.15418974869023643374053993627D+03,d77  = -0.23152937917604549567536039109D+03,d78  =  0.35763911791061412378285349910D+03,d79  =  0.93405324183624310003907691704D+02,d710 = -0.37458323136451633156875139351D+02,d711 =  0.10409964950896230045147246184D+03,d712 =  0.29840293426660503123344363579D+02,d713 = -0.43533456590011143754432175058D+02,d714 =  0.96324553959188282948394950600D+02,d715 = -0.39177261675615439165231486172D+02,d716 = -0.14972683625798562581422125276D+03)
...
!C --- INITIAL PREPARATIONS   
      ...
      HMAX=ABS(HMAX)     
      IORD=8  
      IF (H.EQ.0.D0) H=HINIT(N,FCN,X,Y,XEND,POSNEG,K1,K2,K3,IORD,HMAX,ATOL,RTOL,ITOL,RPAR,IPAR)
 !    &                       HMAX,ATOL,RTOL,ITOL,RPAR,IPAR)
      ...
      IF (IOUT.NE.0) THEN 
          IRTRN=1 
          HOUT=1.D0
          CALL SOLOUT(NACCPT+1,XOLD,X,Y,N,CONT,ICOMP,NRD,RPAR,IPAR,IRTRN,XOUT)
...
!C --- THE TWELVE STAGES
  ...
  28  Y1(I)=Y(I)+H*(A81*K1(I)+A84*K4(I)+A85*K5(I)+A86*K6(I)+A87*K7(I))  
      ...
      DO 29 I=1,N 
  29  Y1(I)=Y(I)+H*(A91*K1(I)+A94*K4(I)+A95*K5(I)+A96*K6(I)+A97*K7(I)+A98*K8(I))
!     &   +A98*K8(I))
      CALL FCN(N,X+C9*H,Y1,K9,RPAR,IPAR)
      DO 30 I=1,N 
  30  Y1(I)=Y(I)+H*(A101*K1(I)+A104*K4(I)+A105*K5(I)+A106*K6(I)+A107*K7(I)+A108*K8(I)+A109*K9(I))
!     &   +A107*K7(I)+A108*K8(I)+A109*K9(I))
      CALL FCN(N,X+C10*H,Y1,K10,RPAR,IPAR)
      DO 31 I=1,N 
  31  Y1(I)=Y(I)+H*(A111*K1(I)+A114*K4(I)+A115*K5(I)+A116*K6(I)+A117*K7(I)+A118*K8(I)+A119*K9(I)+A1110*K10(I))
!     &   +A117*K7(I)+A118*K8(I)+A119*K9(I)+A1110*K10(I))
      CALL FCN(N,X+C11*H,Y1,K2,RPAR,IPAR)
      XPH=X+H
      DO 32 I=1,N 
  32  Y1(I)=Y(I)+H*(A121*K1(I)+A124*K4(I)+A125*K5(I)+A126*K6(I)+A127*K7(I)+A128*K8(I)+A129*K9(I)+A1210*K10(I)+A1211*K2(I))
...
!C ------- STIFFNESS DETECTION
 ...
 64         CONTINUE  
            ...
            IF (HLAMB.GT.6.1D0) THEN
               ...
               IASTI=IASTI+1  
               IF (IASTI.EQ.15) THEN
                  IF (IPRINT.GT.0) WRITE (IPRINT,*),' THE PROBLEM SEEMS TO BECOME STIFF AT X = ',X  
...
!C ---     THE NEXT THREE FUNCTION EVALUATIONS
            DO 51 I=1,N 
  51           Y1(I)=Y(I)+H*(A141*K1(I)+A147*K7(I)+A148*K8(I)+A149*K9(I)+A1410*K10(I)+A1411*K2(I)+A1412*K3(I)+A1413*K4(I))
!    &            +A149*K9(I)+A1410*K10(I)+A1411*K2(I)+A1412*K3(I)
!     &            +A1413*K4(I))
            CALL FCN(N,X+C14*H,Y1,K10,RPAR,IPAR)
            DO 52 I=1,N 
  52           Y1(I)=Y(I)+H*(A151*K1(I)+A156*K6(I)+A157*K7(I)+A158*K8(I)+A1511*K2(I)+A1512*K3(I)+A1513*K4(I)+A1514*K10(I))
!     &            +A158*K8(I)+A1511*K2(I)+A1512*K3(I)+A1513*K4(I)
!     &            +A1514*K10(I))
            CALL FCN(N,X+C15*H,Y1,K2,RPAR,IPAR)
            DO 53 I=1,N 
  53           Y1(I)=Y(I)+H*(A161*K1(I)+A166*K6(I)+A167*K7(I)+A168*K8(I)+A169*K9(I)+A1613*K4(I)+A1614*K10(I)+A1615*K2(I))
...
!C --- FAIL EXIT
  ...
  78  CONTINUE
      IF (IPRINT.GT.0) WRITE(IPRINT,979)X   
      IF (IPRINT.GT.0) WRITE(IPRINT,*),' MORE THAN NMAX =',NMAX,'STEPS ARE NEEDED' 
!     &     ' MORE THAN NMAX =',NMAX,'STEPS ARE NEEDED' 
 ...
 979  FORMAT(' EXIT OF DOP853 AT X=',E18.4) 
      ...
      END 
!C
      FUNCTION HINIT(N,FCN,X,Y,XEND,POSNEG,F0,F1,Y1,IORD,HMAX,ATOL,RTOL,ITOL,RPAR,IPAR)
!     &                       HMAX,ATOL,RTOL,ITOL,RPAR,IPAR)
!C ----------------------------------------------------------
...
!C --- BY USING "CONTD8", THE CONTINUOUS COLLOCATION SOLUTION
        ...
        double precision::S_ff,xxxcur(14)
	    
        IF (NR.EQ.1) THEN
           !WRITE (6,99) X,Y(1),Y(2),Y(3),Y(4),NR-1
           !WRITE (6,99) X,Y(1),Y(2),Y(3),Y(4),Y(5),Y(6),Y(7),Y(8),Y(9),Y(10),Y(11),Y(12)
 ...
 10        CONTINUE
           IF (X.GE.XOUT) THEN
                        ...
                        do iii=1,14
                            xxxcur(iii)=CONTD8(iii,XOUT,CON,ICOMP,ND)    
                        enddo
		
					
                          
						WRITE (7,99),XOUT,CONTD8(1,XOUT,CON,ICOMP,ND),CONTD8(2,XOUT,CON,ICOMP,ND),CONTD8(3,XOUT,CON,ICOMP,ND),CONTD8(4,XOUT,CON,ICOMP,ND),&
									&CONTD8(5,XOUT,CON,ICOMP,ND),CONTD8(6,XOUT,CON,ICOMP,ND),CONTD8(7,XOUT,CON,ICOMP,ND),CONTD8(8,XOUT,CON,ICOMP,ND),&
									&CONTD8(9,XOUT,CON,ICOMP,ND),CONTD8(10,XOUT,CON,ICOMP,ND),CONTD8(11,XOUT,CON,ICOMP,ND),CONTD8(12,XOUT,CON,ICOMP,ND),&
									&CONTD8(13,XOUT,CON,ICOMP,ND),CONTD8(14,XOUT,CON,ICOMP,ND),&
									&S_ff(xxxcur)
 ...
 99     FORMAT(16ES20.12)       
        ...
        double precision function S_ff(xxx)
        use mod_1
        double precision,intent(in)::xxx(14)        
        double precision::pv
		
!*************************************************************************	
		
		pv=dsqrt((xxx(8)**2.D-0)+(xxx(9)**2.D-0))
		
        
        S_ff=-((xxx(12))*1.D0/w)+(1.D0/xxx(5))*pv
